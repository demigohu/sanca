...
export function useContribute(poolAddress: string | undefined) {
  ...
  const contributionPerPeriod = data?.pool.contributionPerPeriod;
  ...
  async function contribute() {
    ...
    setError(null);
    try {
      const txHash = await invoke(poolAddress, 'contribute', [
  ...
  return { contribute, contributionPerPeriod, hash, isPending, isConfirming, isSuccess, error };
}
